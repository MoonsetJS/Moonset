...
export class HiveTaskBuilder {
    ...
    static createHiveTask(scope:cdk.Construct, hive:IHive, idSeq:string):sfn.Task {
        let s3File;
        if (hive.sqlFile) {
            s3File = hive.sqlFile;
            if (!hive.sqlFile.startsWith(s3Prefix)) {
                s3File = new FileAsset(scope, this.buildSqlFileID(idSeq), {
                    path: hive.sqlFile,
                }).getS3Path();
            }
        } else if (hive.sql) {
            s3File = new StringAsset(scope, this.buildSqlFileID(idSeq), {
                content: hive.sql,
            }).getS3Path();
        } else {
        ...
        return this.buildHiveTask(scope, s3File, this.buildTaskID(idSeq));
    }

    private static buildSqlFileID(seq:string):string {
        return 'op-' + seq + 'sql'
    }

    private static buildTaskID(seq:string):string {
        return 'op-' + seq + 'HiveTask'
    }
